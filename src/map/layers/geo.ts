...
/**
 * 描邊（casing）的參數。**顏色是深色不是白色，那是量出來的。**
 *
 * 交通軸線幾乎全程走西部平原，而 NLSC 通用電子地圖在那裡是**白底加淡粉紅公路**
 * ——白色描邊疊在白底上等於沒有加東西。同一相機、同一份圖磚、直接讀 canvas 原始
 * 像素實測「高鐵主線 vs 其周圍底圖」的色差中位數：
 *
 *   白 `#ffffff`      ΔE 18.1（5% 的線幾乎融進底圖、只有 62% 高於 15）
 *   深 `#33322e`      ΔE 29.4（1%、91%）  ← 現行
 *   中灰 `#6b6a64`    ΔE 29.0（1%、91%）
 *
 * 順帶兩個實測結論：**寬度幾乎不影響效果**（描邊 3.2 與 4.8 同分），真正起作用的
 * 是顏色；國道一號同時也從 41.1 升到 48.6。
 *
 * ⚠️ **深色描邊會把線的顏色壓濁**，因為光暈透過反鋸齒滲進細線裡——高鐵的朱紅
 * （L 0.72，很亮）在 2.2px 線寬下會讀成深褐色，而「七條各自一色」正是這一層的重點。
 * 補償方式是**把線加粗**（交通軸線因此用 3.2 而不是預設的 1.4），核心色的像素佔比
 * 提高之後光暈就壓不動它。**改動描邊顏色或線寬之後要重看一次實際畫面**，那個代價
 * 目前沒有可靠的量化指標（試過的分離度指標會因為分類門檻大於待測距離而失真）。
 *
 * 不透明度略低於 1，讓底圖透一點出來，描邊才不會在低縮放時看起來像一條實心黑線。
 */
const CASING_COLOR = "#33322e";
