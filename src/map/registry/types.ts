...
export type DerivedId =
  ...
  /**
   * 臺灣周邊**板塊名的標註錨點**：名稱與 id 取自 `tw-plates.geojson`，
   * 每一塊擺在哪裡則是 `resolve.ts` 裡一份手訂的錨點表（比照洲名）。
   *
   * ⚠️ 理由跟洲名**不完全相同**，兩個都要記著：洲名是「一洲有 240 塊多邊形」，
   * 這裡每塊板塊都只有一塊多邊形，踩到的是另一半——**標註錨點是逐圖磚算的**。
   * 實測臺灣主題的預設視角（zoom 7）上，揚子板塊跨了四張圖磚就印出四次
   * 「揚子板塊」、沖繩板塊兩次。把 `label.maxzoom` 壓到 6 以下確實能擋掉，
   * 但那等於在這個主題最常用的尺度上讓板塊變成沒有名字的色塊。
   */
  | "tw-plate-labels";
...
export type LayerRender =
  ...
  | {
       ...
       *
       * ⚠️ **同一塊面也會隨著 zoom 變大而被重複標註**，因為那個錨點是**逐圖磚**算的：
       * 一塊面跨越幾張圖磚，就會拿到幾個標註。實測 `tw-eez` 在 zoom 5 是每片 1 個、
       * zoom 6 變 4 個、zoom 8 的臺灣那一片有 6 個「臺灣」散在海上。板塊沒踩到是因為
       * 它 `maxzoom: 8` 而且實際只在 zoom 1–3 看。**這是 maplibre 的行為，不是資料問題**
       * ——要壓下來就用下面的 `maxzoom` 把標註限制在這一層真正會被讀的縮放範圍。
       *
       * 目前有板塊與專屬經濟海域兩層用到。
       */
      label?: {
        ...
        minzoom?: number;
        /**
         * 高於這個 zoom 不畫標註。用來壓上面那個**逐圖磚重複標註**的問題：
         * 放大之後面已經鋪滿畫面、名字也早就讀過了，同一個名字重複五六次只是雜訊。
         *
         * ⚠️ 它**只擋標註，不擋面**——圖層層級的 `minzoom`／`maxzoom` 會讓整層消失，
         * 而線與面是走 `fitBounds` 的、算出來的 zoom 跟畫布大小相依，很容易變成
         * 「相機飛對了但畫面空白」的靜默失敗（見 CLAUDE.md 縣市政府那個坑）。
         */
        maxzoom?: number;
      };
    }
...
export type DetailSpec =
  ...
  | {
       ...
       * 活動斷層（37 條共用同一段說明，跟圖層抽屜那一列完全重複，卡片有大半高度在
