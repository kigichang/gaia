...
export const worldTheme: ThemeDefinition = {
  ...
  layers: [
    ...
    //     （線 2 筆）的圖層定義在 git 裡（本檔上一版），要復原就是把那兩個物件
    //     貼回這個位置。
    ...
    //     產生（那個資料集刻意沒動）。
    //   - 重新設計前請先讀 CLAUDE.md 的「世界櫥窗」那幾節：為什麼是兩個圖層而不是
    //     一個、三座火山為什麼併進點圖層、圓點名額上限、以及「講不出憑據的『最』
    //     不收」這條判準，結論都記在那裡。
    //
    // 目前這一組底下只剩「作者精選」兩層。⚠️ 少了「世界之最」之後，世界主題的
    // 圓點圖層從五個回到四個，正好等於 `MAX_ACTIVE_BY_KIND.circle`——核取方塊
    // 暫時不會再出現勾滿變 disabled 的情形，那是下架的連帶結果，不是上限被改過。
    {
      id: "world-picks",
      label: "作者精選",
      group: "世界櫥窗",
      ...
      render: { kind: "circle" },
      /**
       * ⚠️ 同樣沿用 `place` 藍（「藍點＝地圖上一個有詳情卡的地點」）：POINT 色票
       * 已經是 all-pairs 全過的飽和狀態，而附屬／櫥窗類的點一律用藍是本站記錄
       * 在案的規則（見 thematicColors.ts）。
       */
      colorRole: "place",
      detail: { type: "geo", collection: "world-picks" },
      /**
       * 依 `category` 分組（⚠️ `groupBy` 依序切、不排序，同一類在 geojson 裡必須
       * 連續）。取景一樣寫在 geojson 的 `properties.zoom` 上逐筆決定——這一層的
       * 尺度差很大（維蘇威是一座山，理查特結構直徑四十公里），一個 `browse.zoom`
       * 蓋不住。
       */
      browse: { groupBy: "category" },
      description:
        "編者挑出來、值得在地圖上找一找的地方。義大利的維蘇威火山——西元 79 年那場" +
        "噴發把山腳下的龐貝城整個埋起來，也留下人類第一份詳細的火山噴發目擊記錄；" +
        "撒哈拉沙漠裡的理查特結構（「撒哈拉之眼」）——那圈四十公里寬的靶心不是" +
        "隕石坑，是被侵蝕削平的巨大穹丘；南太平洋的尼莫點——離所有陸地最遠的一點，" +
        "退役的太空船都往那裡送；荷姆茲海峽——一條 33 公里寬的水道，全世界五分之一的" +
        "石油從這裡出去；麻六甲海峽——那些油北上東亞的下一個窄口，深水航道只剩 2.8 公里；" +
        "日本小豆島的土渕海峽——只有 9.93 公尺寬，「世界最窄的海峽」" +
        "這個頭銜不是量出來的，是官方登記出來的；還有三個湖——死海、裏海與貝加爾湖，" +
        "各自把「湖」這件事推到一個極端。",
      notes: [
        "⚠️ 這一層是編輯選集，不是排名，也不會補完。收錄的界線是：課本、新聞或科普讀物會直接叫出名字，在地圖上指得出明確的位置，而且那個地方本身就講得出一堂課。",
        "⚠️ 這一層只放**單點**的項目；有範圍的（東非大裂谷、百慕達三角）在同一組的「作者精選・範圍」，兩層的收錄判準相同，分開只是因為一個圖層只能有一種幾何。",
        ...
        "⚠️ 土渕海峽最窄處只有 9.93 公尺，而本站的地圖最多只能放大到 zoom 16（那個緯度下約 2 公尺一個像素），所以那條水道實測只有 5 個像素寬，而且最窄處正好架著橋、上面還壓著這一層自己的圓點——看起來會像水道在那裡斷掉。底圖是向量圖磚不是空拍影像，要看清楚請往兩側找那條藍色細線。",
    ...
    {
      id: "world-picks-areas",
      label: "作者精選・範圍",
      group: "世界櫥窗",
      status: "ready",
      /**
       * 「作者精選」裡有**範圍**而不是單點的項目。跟 `world-picks` 分成兩層純粹是
       * 幾何限制（`LayerRender` 一個圖層只能一種幾何），內容判準與內容 collection
       * 都跟它共用——`world-picks` 底下 id 唯一就夠（比照「世界之最」那兩層）。
       */
      source: { type: "remote", path: "data/geo-manual/world-picks-areas.geojson" },
      /**
       * ⚠️ **虛線是必要的，不是裝飾。** 這一層畫的每一條線都是**示意**的，實線會讓
       * 它們看起來像一條可以照著量的真實界線。兩筆的理由不同，但結論一樣：
       *
       * - **百慕達三角根本沒有官方界線**（美國海軍與海岸防衛隊明講它不在任何官方
       *   海圖上），畫成實線正好會坐實那張卡要拆穿的東西。
       * - **東非大裂谷是真實的地形，但它不是一條線**——真正的裂谷是一組數十公里寬
       *   的地塹與斷層帶，畫成一條線就已經是示意了（比照世界主要山脈的中軸線）。
       *
      ...
      browse: { groupBy: "category" },
      /**
       * ⚠️ `maxzoom: 8`：這幾條線再放大也只是幾條線，而這一層畫的是一個**概念**的
       * 範圍。讓它在區域尺度就收掉，避免看起來像一條可以逐段追下去的界線。
       */
      maxzoom: 8,
      description:
        "「作者精選」裡有範圍的項目：東非大裂谷——一塊大陸正在裂開，而且看得到過程；" +
        "以及百慕達三角——一個沒有官方界線的「地方」，拿來練習怎麼查證一個" +
        "聽起來很像真的說法。",
      notes: [
        "⚠️ 這一層的線都是示意的，但原因不同。百慕達三角**沒有官方界線**（不存在於任何官方海圖上），各家畫法不同，面積從 130 萬到 390 萬平方公里都有；東非大裂谷**是真實的地形**，但真正的裂谷是一組數十公里寬的地塹與斷層帶，畫成一條線本身就是示意。兩筆都標了示意。",
