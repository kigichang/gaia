...
export const worldTheme: ThemeDefinition = {
  ...
  layers: [
    ...
    // collection（`world-superlatives`），id 就是它在講的那個「最」。
    {
      ...
      status: "ready",
      /**
       * 兩個座標都是人工轉錄的官方數字（比照臺灣海峽中線），所以放 `geo-manual/`：
      ...
      /**
       * 兩筆而已，不必分組。⚠️ 這裡**刻意不填 `zoom`**：兩筆的尺度差了五級
       * （聖母峰要看整條喜馬拉雅、日和山是一顆 3 公尺高的沙丘），所以取景寫在
       * geojson 的 `properties.zoom` 上逐筆決定（`ThemeMapPage` 優先讀那一個）。
       */
      browse: {},
      description:
        "海拔最高的山峰（聖母峰 8,848.86 公尺）與海拔最低的山（日和山 3 公尺）。" +
        "兩個極端差了將近九千公尺，但兩座都叫「山」——這一層要問的是「山」到底怎麼量、怎麼認定。",
      notes: [
