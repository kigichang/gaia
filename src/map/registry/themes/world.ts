...
export const worldTheme: ThemeDefinition = {
  ...
  layers: [
    ...
    //     貼回這個位置。
    //   - 內容檔 `src/content/geo/world-superlatives/`（6 份）與幾何
    ...
    //     產生（那個資料集刻意沒動）。
    //   - ⚠️ **中洋脊 2026-09 已經以獨立圖層回到「地體構造」**（`world-mid-ocean-ridge`）：
    //     幾何另出一份產物、圖徵 id 是 `mid-ocean-ridge`（**刻意不撞** 這一層那筆
    //     `longest-undersea-range`，理由見 build-geodata.mjs），說明卡也搬去了新的
    //     collection，所以這裡的內容檔從 7 份變 6 份。重新設計時**不要重複收它**。
    ...
    },
    {
       ...
       * （`featureIds` 是過濾、不重排）。兩筆依經度由西往東排（希克蘇魯伯 −89.5°、
       * 理查特 −11.4°），比照「海峽」與「湖泊」那兩層的既有規則。
       *
       * ⚠️ **這兩筆是一組，改一張卡要順手看另一張**：理查特結構看起來就像隕石坑
       * 卻不是，希克蘇魯伯完全看不出來卻是。兩張卡都在講「判斷不靠外形，靠只有
       * 撞擊才留得下的證據」，拆開任何一筆都會讓另一筆的論證少掉一半。
       */
      featureIds: ["chicxulub-crater", "richat-structure"],
      render: { kind: "circle" },
      colorRole: "place",
      exemptFromMaxActive: true,
      detail: { type: "geo", collection: "world-picks" },
      browse: {},
      description:
        "編者挑出來、值得在地圖上找一找的地形與地質現場。墨西哥猶加敦半島外海的" +
        "希克蘇魯伯隕石坑——地表上什麼都看不到，卻是六千六百萬年前結束恐龍時代的" +
        "那個撞擊坑；撒哈拉沙漠裡的理查特結構（「撒哈拉之眼」）剛好相反，那圈" +
        "四十公里寬的靶心不是隕石坑，是被侵蝕削平的巨大穹丘。",
      notes: [
        "⚠️ 這一層是編輯選集，不是排名，也不會補完。收錄的界線是：課本、新聞或科普讀物會直接叫出名字，在地圖上指得出明確的位置，而且那個地方本身就講得出一堂課。",
        "⚠️ 理查特結構直徑四、五十公里，這一層畫的那個點是它的中心，不是一個可以指認的地物；那圈同心圓要放大到 zoom 9 以上、並打開地形陰影才看得出來（本站的世界底圖是向量圖磚，不是衛星影像）。",
        "⚠️ 希克蘇魯伯隕石坑直徑約 180 公里，這一層畫的那個點是坑的中心，落在猶加敦半島北岸外的海面上。這個坑整個埋在地下約一公里，所以跟理查特結構相反——本站的等高線與地形陰影在這裡什麼都畫不出來（實測從北岸往南 100 公里，地面只從 0 爬到約 27 公尺）。地表上唯一的線索是沿著坑緣排成一圈的天然井，那份資料本站沒有收。",
      ],
      sources: [
        "維基百科 希克蘇魯伯隕石坑",
        "ECORD IODP 第 364 航次 希克蘇魯伯 K-Pg 撞擊坑",
        "維基百科 理查特結構",
        "IUGS 全球地質遺產 理查特結構",
        "NASA 地球觀測站 理查特結構",
      ],
    ...
    },
    {
      ...
      group: "地體構造",
      status: "ready",
      source: { type: "remote", path: "data/geo/quakes-strongest.geojson" },
      /**
       * **同一個 `hazard` 中性色，但畫得「更深」**：不透明度 0.9、半徑加大、
       * 加一圈白框。使用者要的是「把這 18 次標出來」，不是另一個類別，所以刻意
       * 不給新色相——那也省下一次色票驗證。做法逐字沿用臺灣主題的
       * 「重大地震」（`tw-quakes-major`）疊在「臺灣地震」上的那一組參數。
       *
       * ⚠️ **半徑下限 5 是「看不看得見」的門檻，不要往回調。** 這一層預設要在
       * 主題視角（zoom 1.8）就讀得出「最強的幾次全部沿著隱沒帶排列」，而母圖層
       * 「全球地震帶」踩過那個坑（1.5 px 的灰點＝「勾了圖層但畫面上沒東西」）。
       * ⚠️ 反過來，白框在這一層是**必要**的（母圖層刻意不畫）：18 顆點常常正好
       * 疊在母圖層最密的那幾段上，沒有外框就融進那團灰裡。
       */
      render: {
        kind: "circle",
        radius: ["interpolate", ["linear"], ["get", "mag"], 8.4, 5, 9.5, 12],
        strokeWidth: 1.5,
        opacity: 0.9,
      },
      colorRole: "hazard",
      /**
       * ⚠️ **這一層不佔圓點名額**（比照「世界紀錄熱帶氣旋路徑」那條既有判準）：
       * 它用的是**非分類的固定角色色** `hazard`，跟同組的「全球地震帶」同色，
       * 多開一層畫面上一個新色相都沒有增加。而世界主題已經有五個 ready 的圓點
       * 圖層在搶四個名額（全球地震帶、火山帶、世界重要城市、世界人口分布、
       * 古文明發源地），不豁免的話「把最強的幾次標出來」就得跟它要對照的
       * 「全球地震帶」「火山帶」二選一——那正是這一層存在的理由。
       * ⚠️ 豁免不是上限失效：其餘五個圓點圖層照樣四個封頂。
       */
      exemptFromMaxActive: true,
      /**
       * ⚠️ **不共用 `tw-quakes` 那張 `QuakeCard`**（`detail: { type: "quake" }`）：
       * 那張卡只講規模、震源深度與震央經緯度，而這 18 筆每一筆的重點都是
       * 「它教了我們什麼」——矩震級是為了 1960 智利才發明的、卡斯凱迪亞是靠日本的
       * 「孤兒海嘯」定年的、2012 那次是走向滑移所以幾乎沒有海嘯。那些只有內容檔
       * 寫得下。
       */
      detail: { type: "geo", collection: "quakes-strongest" },
      /**
       * 18 筆、每筆都有名字與日期，是一份讀得懂的清單（母圖層「全球地震帶」
       * 2,800 個沒有名字的點刻意沒有 browse）。
       * ⚠️ `zoom: 5` 是實測的：這些震央幾乎都在外海，取景要同時看得到震央與
       * 最近的那段海岸，才看得出「它為什麼會淹到那裡」。
       */
      browse: { zoom: 5 },
      description:
        "有紀錄以來規模最大的 18 次地震，收錄名單取自維基百科〈地震列表〉的" +
        "「震級最強地震」。除了 1668 年郯城與 1950 年墨脫兩次在陸上，其餘全部" +
        "落在隱沒帶——把它跟「板塊邊界」疊起來看，那條線就是規模的上限在哪裡。",
      notes: [
        "⚠️ 規模一律採 USGS 地震目錄的矩震級，跟維基百科那張表有四筆對不起來：2004 年印度洋（USGS 9.1、表上 9.3）、1700 年卡斯凱迪亞（9.0／9.2）、2005 年蘇門答臘（8.6／8.7）、2007 年蘇門答臘（8.4／8.5）。卡片上兩個數字都會列出來——「最強」取決於誰量、什麼時候量，那正是這一層要教的事。",
        "⚠️ 也因此清單的順序（規模由大到小）跟維基那張表的排名不完全一樣，2007 年那次會排到最後。",
        "⚠️ 1575 年瓦爾迪維亞、1668 年郯城、1755 年里斯本、1833 年蘇門答臘四次發生在儀器地震學誕生之前，USGS 目錄裡沒有它們。這四筆的位置與規模是從史料、地方志、珊瑚微環礁的抬升量推估出來的，卡片上都標了資料來源，誤差遠大於其他十四筆。",
        "⚠️ 1900 年以前的目錄本來就不完整：規模再大，只要發生在沒有人記錄、也沒有留下地質證據的地方，就不會出現在任何一張「最強」的表上。這張表是「我們知道的最強」，不是「發生過的最強」。",
        "⚠️ 畫的是震央（地面上的一個點），不是破裂的範圍。2004 年那次的斷層破裂長達 1,200–1,300 公里，一個圓點畫不出來。",
      ],
      sources: ["維基百科 地震列表", "USGS 地震目錄"],
    },
    {
      id: "plates",
      label: "板塊",
    ...
    },
    {
       ...
       * ## 為什麼排在「板塊邊界」正後面
       *
       * 中洋脊**是**張裂型邊界的一部分（Bird 2003 的 `OSR` 段落），這一層跟上一層
       * 畫的是同一批座標，差別只在框架：上一層問「板塊怎麼動」，這一層問「地球上
       * 最長的山脈在哪裡」。抽屜裡挨著擺，讀者才會順手兩層一起勾——這一層的說明卡
       * 最後一條「對照重點」正是叫他這麼做。
       *
       * ⚠️ **只取 `OSR`，不含 `CRB`（大陸裂谷）。** 張裂型是 OSR ＋ CRB 兩類，而
       * 東非大裂谷、貝加爾裂谷在陸地上，不是海底山脈。所以這一層在東非與貝加爾湖
       * 那裡**不會有線**，那不是漏畫（判準寫在 scripts/lib/plates.mjs 的
       * `MID_OCEAN_RIDGE_CLASS`）。
       *
       * ⚠️ 它跟下架中的「世界之最・山脈」是**兩份產物、兩個 id**（那一層的中洋脊
       * 是 `longest-undersea-range`）。id 不能撞：`highlightIds` 是一份跨圖層的扁平
       * 清單，撞了的話那一層將來復原時兩邊會互相加粗。
       */
      source: { type: "remote", path: "data/geo/world-mid-ocean-ridge.geojson" },
      /**
       * ## ⚠️ 這一層**沒有沿線標註**，而且不要再加回來——是量過的，不是漏掉
       *
       * 站上其他線圖層一律帶 `label`（河流、山脈、洋流、板塊邊界），所以這裡的缺席
       * 看起來像忘了寫。**三種放置方式都實測過，沒有一種可用**（1920×873、production
       * build、`queryRenderedFeatures` 數實際放置數）：
       *
       * | 放置方式 | zoom 1.8（本主題預設視角） | zoom 3 | zoom 5 |
       * |---|---|---|---|
       * | `line`（站上線圖層的預設，連 `maxAngle: 150` 都試過） | **0** | 0 | 0 |
       * | `line-center` | **0** | 3 | 4 |
       * | `point` | **594 段各放一個**，整片海面都是「中洋脊」 | — | — |
       *
       * 原因在幾何而不在參數：這一層是 594 段、**平均只有 2.78 個點**的
       * MultiLineString（脊軸被一段段轉形斷層錯開，`mergeStepRuns` 在非 OSR 的段落
       * 就切斷串接——那是對的，接起來會畫出橫跨錯移的假線）。`symbol-placement: line`
       * 需要**單一一段**長到擺得下整串文字，而這裡沒有任何一段夠長，所以調
       * `symbol-spacing`／`text-max-angle` 一點用都沒有（河川那次的教訓在這裡不適用，
       * 那是「線太彎」，這是「線太短」）。
       *
       * 少了標註可以接受，理由是這一層**只有一筆圖徵**：圖層抽屜那一列與左下角圖例
       * 都寫著「中洋脊」，識別本來就不靠地圖上的字（洋流要標是因為有 18 條同色的線
       * 要分辨，這一層沒有這個問題）。
       *
       * ⚠️ 真的要在圖上寫出名字，唯一可行的做法是比照大洲名與臺灣板塊名，加一個
       * `radius: 0` 的**附屬錨點圖層**——那需要一份 `geo-manual/` 的錨點幾何，而且
       * 要決定「一條繞地球一圈的線該把名字放在哪」。不要用 `render.label` 再試一次。
       */
      render: { kind: "line", width: 2.6 },
      /**
       * ⚠️ **不是世界主要山脈的 `mountain` 紫**，即使兩者都是山脈、即使這一層下架前
       * 用的就是它。山脈紫當初把洋流排除在同框名單外的理由是「山脈全部在陸上」，
       * 而中洋脊全程在海上——實測山脈紫對寒流藍的 CVD ΔE 只有 2.9（protan 下是
       * 同一個顏色）。掃描條件與量測值見 thematicColors.ts 的 `MID_OCEAN_RIDGE_COLOR`。
       */
      colorRole: "midOceanRidge",
      detail: { type: "geo", collection: "world-mid-ocean-ridge" },
      /** 只有一筆圖徵，但仍然要有可點清單——它同時是這一層進搜尋索引的條件。 */
      browse: {},
      /**
       * ⚠️ `maxzoom: 6` 跟「板塊邊界」與「世界主要山脈」是同一個理由：一個像素在
       * zoom 6 約 0.04°，正好是產物簡化容差（0.02°）的兩倍，再放大那條線就會變成
       * 一條假的精確稜線。
       */
      maxzoom: 6,
      description:
        "地球上最長的山脈，全長約 65,000 公里，九成以上在海面下。兩塊板塊在這裡分開、" +
        "地函物質上湧成新的海洋地殼——把「板塊邊界」一起打開，這條線會與張裂型邊界重疊；" +
        "再打開「全球地震帶」，淺源地震正沿著它排列。",
      notes: [
        "⚠️ 這一層只畫海洋擴張脊（Bird (2003) 模型裡的 OSR），**不含大陸裂谷**（東非大裂谷、貝加爾裂谷）。那些同屬張裂型邊界，但在陸地上，不是海底山脈——所以東非與貝加爾湖那裡看不到這條線，那不是漏畫。",
        "⚠️ 這條線與「板塊邊界」的張裂型邊界畫的是同一件事，兩層一起打開時會重疊。這一層是淺紫實線、板塊邊界是橘色虛線。",
        "⚠️ 中洋脊不是一條線而是一道隆起：脊軸兩側的隆起可以寬達上千公里，圖上那條線是模型的簡化。長度的 65,000 公里採美國國家海洋暨大氣總署的公布值，不是從圖上這條線量出來的。",
      ],
      sources: [
        "美國國家海洋暨大氣總署 中洋脊",
        "維基百科 洋中脊",
    ...
    },
    {
      ...
      group: "地體構造",
      status: "ready",
      source: { type: "remote", path: "data/geo-manual/volcano-awards.geojson" },
      /**
       * ⚠️ **這一層跟「火山帶」的關係，逐字比照「規模最強地震」疊在「全球地震帶」
       * 上的那一組作法**：同一個 `volcano` 洋紅、但畫得更大、更實、加一圈白框。
       * 使用者要的是「把這六座標出來」，不是另一個類別——所以刻意不給新色相，
       * 那也省下一次色票驗證（POINT 色票已經是 all-pairs 全過的飽和狀態）。
       *
       * ⚠️ 半徑 7 是「看不看得見」的門檻，不要往回調：這一層預設要在主題視角
       * （`center [0,10]`、zoom 1.8）就讀得出六個點各在哪一洲。母圖層「火山帶」
       * 的 3.2 是給 1,214 座密集點用的，六顆點照抄那個值就會沉進去。
       * ⚠️ 反過來，白框在這一層是**必要**的（母圖層刻意不畫）：五顆點正好疊在
       * 環太平洋最密的那一段上，沒有外框就融進那團洋紅裡。
       */
      render: { kind: "circle", radius: 7, strokeWidth: 1.5, opacity: 0.95 },
      colorRole: "volcano",
      /**
       * ⚠️ **這一層不佔圓點名額**（比照「規模最強地震」與世界櫥窗那九層的既有
       * 判準）：它用的是**非分類的固定角色色** `volcano`，跟同組的「火山帶」同色，
       * 多開一層畫面上一個新色相都沒有增加。而世界主題已經有五個吃名額的圓點
       * 圖層（全球地震帶、火山帶、世界重要城市、世界人口分布、古文明發源地），
       * 不豁免的話「把這六座標出來」就得跟它要對照的「火山帶」二選一——那正是
       * 這一層存在的理由。
       * ⚠️ 豁免不是上限失效：其餘五個圓點圖層照樣四個封頂。
       */
      exemptFromMaxActive: true,
      /**
       * ⚠️ **不共用「火山帶」那個 `volcanoes` collection。** 那一層的卡片講的是
       * GVP 那一筆的類型、海拔與最後噴發年代（1,214 座共用一套欄位），而這六座
       * 每一座的重點都是「它憑什麼拿這個獎」——超級火山的門檻怎麼定、預報怎麼
       * 救下五千條人命、1792 年殺人的是山崩不是熔岩。那些只有內容檔寫得下。
       */
      detail: { type: "geo", collection: "volcano-awards" },
      /**
       * 六筆、每筆都有名字，是一份讀得懂的清單。
       * ⚠️ 取景寫在 geojson 的 `properties.zoom` 上、不是這裡的 `browse.zoom`：
       * 六筆的尺度差很多（黃石的破火山口 70 公里寬、櫻島只有 10 公里），
       * 而 `ThemeMapPage` 對點圖層是 `feature.properties.zoom || browse.zoom || 11`
       * ——逐筆決定才對（比照世界櫥窗那九層的既有作法）。
       */
      browse: {},
      description:
        "兩份公開名單併起來的世界著名火山：大屯火山觀測站（TVO）「與火山近距離」" +
        "那一頁挑出來的六座，以及國際火山學與地球內部化學協會（IAVCEI）在聯合國" +
        "「國際減少自然災害十年」計畫下選出的 16 座「十年火山」。每一座各自因為一件事" +
        "上榜——黃石是規模的極端、皮納圖波是預報的範例、加勒拉斯是一場帶走六位火山學家" +
        "的意外、雷尼爾山的危險藏在山上的冰裡。點任一座可以看它「憑什麼上榜」。",
      notes: [
        "⚠️ 這一層是依兩份公開名單收的教學名單，不是排名，也不會補完：（一）大屯火山觀測站「與火山近距離－世界火山」那一頁；（二）IAVCEI 選出的 16 座「十年火山」（判準是噴發史兇猛、近期活躍，而且旁邊住了很多人）。兩份重疊的三座（櫻島、雲仙岳、塔阿爾）只收一次。",
        "⚠️ 十年火山 16 座在這一層有 15 座：維蘇威火山本站已經收在「世界櫥窗・火山與災害」那一層（那張卡講龐貝城），這裡不重複收錄；阿瓦恰與科里亞克在 IAVCEI 名單上合列為一座，本站畫阿瓦恰那一個點，科里亞克在「火山帶」那一層上找得到（名字寫的是 Koryaksky）。",
        "⚠️ 黃石不在「火山帶」那一層裡：它上次噴發在 64 萬年前，早於全新世（約一萬年），按國際通用的定義不算活火山，所以史密森尼學會的名單裡沒有它。這一層收它正是為了把那條定義畫出來。",
        "⚠️ 除了黃石以外，每一筆的座標與海拔都逐位元取自「火山帶」那一份 GVP 資料，兩層一起打開時洋紅大點與小點會疊在同一個位置，那是刻意的。⚠️ 有幾座在 GVP 名單上的條目名不一樣：櫻島是「姶良破火山口（櫻島）」、泰德峰是「特內里費（Tenerife）」、聖瑪利亞火山是「Santa Maria」、加勒拉斯火山在那一層寫作「加雷拉斯火山」——都是同一座火山系統，不是標錯。",
        "⚠️ 有四座在「火山帶」那一層也各有一張說明卡（埃特納、聖托里尼、皮納圖波、富士山）。兩層一起打開時，點那個座標開出來的是這一層的卡（它畫在上面）；兩張卡的角度不同，那一層講的是 GVP 那一筆的類型與噴發年代，這一層講的是它憑什麼上榜。",
        "⚠️ 黃石那一筆畫的是破火山口的中心（70×45 公里），不是一個指認得出來的地物；其餘各筆是整座火山（或火山島）的代表位置，不是噴發口的精確座標。",
        "⚠️ 本站「板塊」那一層用的是 Bird (2003) 模型，它把課本說的大板塊細分成好幾塊，所以點下去看到的板塊名可能跟課本不同。實測這 18 座各自落在：黃石／雷尼爾山／科利馬／聖瑪利亞＝北美板塊、茂納羅亞＝太平洋板塊、加勒拉斯＝北安地斯板塊、泰德峰／埃特納／尼拉貢戈＝非洲板塊、聖托里尼＝愛琴海板塊、默拉皮／皮納圖波／塔阿爾＝巽他板塊、雲仙岳＝阿穆爾板塊、櫻島＝沖繩板塊、富士山／阿瓦恰＝鄂霍次克板塊、烏拉旺＝南俾斯麥板塊。",
      ],
      sources: [
        "大屯火山觀測站 世界火山",
        "維基百科 十年火山",
