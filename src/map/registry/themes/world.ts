  ...
  layers: [
    ...
    // 暫時不會再出現勾滿變 disabled 的情形，那是下架的連帶結果，不是上限被改過。
    {
      ...
      description:
        ...
        "是官方登記出來的；兩條運河——巴拿馬與蘇伊士，人類自己挖出來的咽喉點，一條" +
        "要把船抬高 26 公尺、另一條一道船閘都不用；兩個海角——合恩角與好望角，運河" +
        "出現之前繞過整塊大陸的那兩個轉角，而它們的名字都比實際的位置有名（好望角" +
        "不是非洲最南端，兩個大洋的分界其實是從海角拉出去的一條經線）；還有三個湖" +
        "——死海、裏海與貝加爾湖，各自把「湖」這件事推到一個極端。",
      notes: [
        ...
        "⚠️ 兩條運河（巴拿馬、蘇伊士）自成一組 `category: \"運河\"`，排在「海峽」後面：它們跟海峽是同一個主題（咽喉點），差別在一個是地形逼出來的、一個是人挖出來的。畫的都是運河中段水道上的一個點，不是任何一端的港口。",
        ...
        "⚠️ 兩個海角（合恩角、好望角）自成一組 `category: \"海角\"`，排在「運河」後面：海峽與運河是主要航線上的咽喉點，海角是那些航線走不通時剩下的那條路。組內依經度由西往東排（合恩角 −67.3°、好望角 18.5°）。",
    ...
    },
    {
       ...
       *
       * ⚠️ **它 2026-08 從「作者精選」搬出來自成一層。** 那六筆原本掛在
       * `world-picks` 底下當一個 `category`，但那一層的定位是「編者挑的地方，
       * 沒有共同的主題」；這六筆相反——它們是**同一個問題的六個答案**，彼此要
       ...
       * 記錄在案的規則（世界重要城市與作者精選共用同一個 hex 就是這條規則）。
       *
      ...
      notes: [
        ...
        "⚠️ 這一層的圓點與「世界重要城市」「作者精選」同色（藍＝地圖上一個有詳情卡的地點）。孟菲斯與開羅只差 23 公里、二里頭與洛陽市只差 20 公里，兩層一起打開時請以圖例與點選後的卡片判讀。",
      ],
      sources: [
        "維基百科 文明的搖籃",
        "維基百科 烏魯克",
