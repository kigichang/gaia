...
export const worldTheme: ThemeDefinition = {
  ...
  layers: [
    ...
    },
    {
      ...
      browse: {},
      description:
        "臺灣火山觀測所（TVO）「與火山近距離」那一頁挑出來的世界著名火山，" +
        "各自因為一件事而出名：黃石是規模的極端、皮納圖波是預報的範例、" +
        "雲仙岳是日本最致命的火山災害、櫻島一年爆發近千次、富士山被列進世界" +
        "文化遺產、塔阿爾則是湖中島中湖的奇特地形。點任一座可以看它「憑什麼上榜」。",
      notes: [
        "⚠️ 這一層是依臺灣火山觀測所那一頁選出來的教學名單，不是排名，也不會補完。同一頁列出的維蘇威火山本站已經收在「世界櫥窗・火山與災害」那一層，這裡不重複收錄。",
        "⚠️ 黃石不在「火山帶」那一層裡：它上次噴發在 64 萬年前，早於全新世（約一萬年），按國際通用的定義不算活火山，所以史密森尼學會的名單裡沒有它。這一層收它正是為了把那條定義畫出來。",
        "⚠️ 其餘五座的座標與海拔逐位元取自「火山帶」那一份 GVP 資料，兩層一起打開時洋紅大點與小點會疊在同一個位置，那是刻意的。⚠️ 櫻島在 GVP 名單上的條目名是「姶良破火山口（櫻島）」——那是同一座火山系統，不是標錯。",
        "⚠️ 黃石那一筆畫的是破火山口的中心（70×45 公里），不是一個指認得出來的地物；其餘五筆是整座火山（或火山島）的代表位置，不是噴發口的精確座標。",
        "⚠️ 本站「板塊」那一層用的是 Bird (2003) 模型，它把課本說的歐亞板塊細分成好幾塊。實測這六座各自落在：黃石＝北美板塊、皮納圖波與塔阿爾＝巽他板塊、雲仙岳＝阿穆爾板塊、櫻島＝沖繩板塊、富士山＝鄂霍次克板塊。點下去看到的板塊名跟課本不同時，先看這一條。",
      ],
      sources: [
        "大屯火山觀測站 世界火山",
        "史密森尼學會 全球火山計畫（GVP）",
