...
export const worldTheme: ThemeDefinition = {
  ...
  layers: [
    ...
    },
    {
       ...
       * （`featureIds` 是過濾、不重排）。兩筆依經度由西往東排（尼莫點 −123.4°、
       * 馬里亞納海溝 142.2°），比照「海峽」那一層的既有規則。
       */
      featureIds: ["point-nemo", "mariana-trench"],
      render: { kind: "circle" },
      colorRole: "place",
      exemptFromMaxActive: true,
      detail: { type: "geo", collection: "world-picks" },
      ...
      description:
        ...
        "最遠的一點，退役的太空船都往那裡送；西北太平洋的馬里亞納海溝——地表最深的" +
        "地方，而「最深」是好幾個對不起來的數字。",
      notes: [
        "⚠️ 這一層是編輯選集，不是排名，也不會補完。收錄的界線是：課本、新聞或科普讀物會直接叫出名字，在地圖上指得出明確的位置，而且那個地方本身就講得出一堂課。",
        "⚠️ 尼莫點是 1992 年用數位海岸線資料算出來的「海洋難達極」，海面上沒有任何地物，也沒有國際機構公告過它的官方座標；換一份海岸線資料重算，位置會有幾公里的出入。這一層畫的是常被引用的那組座標。",
        "⚠️ 馬里亞納海溝畫的是維基百科條目資訊框所載座標處的一個點，不是整條海溝——它長 2,550 公里、最寬 69 公里；最深的挑戰者深淵在這個點東邊數十公里處（1951 年挑戰者二號的測深位置是 11°19′N 142°15′E）。",
        "⚠️ 海溝最深處的數字不只一個：從 1951 年的 10,863 公尺到 1984 年的 11,040.4 公尺都有人測過，維基百科那篇的資訊框寫的是 1957 年那個從未再測得的 11,034 公尺。卡片上採 2019 年維斯科沃下潛實測的 10,928 公尺，並把各次測量的年份與方法一起列出來——那正是這一筆要教的東西。",
        "⚠️ 本站的世界底圖是向量圖磚、不是空拍或水深圖，海溝在底圖上什麼都看不到；要在畫面上看出那條溝，請打開「地形陰影」（全球 DEM 含海底地形），或打開「板塊邊界」看聚合型邊界那條虛線沿著海溝畫過去。",
      ],
      sources: [
        "維基百科 尼莫點",
        "美國國家海洋暨大氣總署 Bloop 聲響",
        "維基百科 馬里亞納海溝",
        "維基百科 挑戰者深淵",
      ],
    },
    ...
    },
    {
      ...
      /**
       * ⚠️ 順序在這裡只是可讀性，真正決定抽屜清單順序的是 geojson 的檔案順序
       * （`featureIds` 是過濾、不重排）。四筆依經度由西往東排（蘇必略湖 −87.1°、
       * 死海 35.5°、裏海 50.7°、貝加爾湖 108.2°），比照「海峽」那一層的既有規則。
       */
      featureIds: ["lake-superior", "dead-sea", "caspian-sea", "lake-baikal"],
      render: { kind: "circle" },
      colorRole: "place",
      exemptFromMaxActive: true,
      detail: { type: "geo", collection: "world-picks" },
      browse: {},
      description:
        "編者挑出來的四個湖——蘇必略湖、死海、裏海與貝加爾湖，各自把「湖」這件事推到" +
        "一個極端：最大的淡水湖面、最低的湖面、最大的面積、最深也最古老。四個「最」" +
        "量的都不是同一件事。",
      notes: [
        "⚠️ 這一層是編輯選集，不是排名，也不會補完。收錄的界線是：課本、新聞或科普讀物會直接叫出名字，在地圖上指得出明確的位置，而且那個地方本身就講得出一堂課。",
        "⚠️ 四個湖畫的都是湖體中央的一點，不是湖的範圍——本站沒有世界湖泊的面圖層，湖的形狀請看底圖。",
        "⚠️ 死海的水位每年還在下降一公尺以上，卡片上的 −440 公尺標的是 2026 年的值；任何一份資料寫的「陸地最低點」數字都要連年份一起看。",
        "⚠️ 蘇必略湖的「世界最大淡水湖」是以面積論的，而且建立在「密西根湖與休倫湖分開算」這個習慣上——兩者水文上其實是同一個湖，合起來的面積比它大。論蓄水量，它排在貝加爾湖與坦干伊喀湖之後。",
        "⚠️ 蘇必略湖的面積本站採美國環保署的 82,100 平方公里（同一張表也是卡片上蓄水量、最大深度與 191 年滯留時間的出處）；中文維基百科寫的是 82,414，兩者差約 300 平方公里。",
      ],
      sources: [
        "維基百科 蘇必略湖",
        "美國環境保護署 五大湖物理特徵",
