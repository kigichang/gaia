...
export const taiwanTheme: ThemeDefinition = {
  ...
  layers: [
    ...
    },
    {
      ...
      /**
       * ⚠️ **這四個參數是實測調出來的，四個都不要照抄別層的預設。** 真實岸線比站上
       * 任何一條線都彎（每一個灣岬都是急彎），`symbol-placement: line` 會大量靜默
       * 拒絕。1492×752 畫布實測「畫面內放得出幾個標註／涵蓋哪幾段」：
       *
       * | 設定 | 全島 z6.6 | 全島 z7.3 | 北海岸 z9.5 | 恆春 z9.5 |
       * |---|---|---|---|---|
       * | spacing 400・150°・11px（預設想法） | 1 | 1 | 1 | 1 |
       ...
       *   不同——180 是「北部海岸」與「恆春半島」在自己的尺度上標不標得出來的分水嶺。
