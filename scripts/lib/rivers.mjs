 ...
 *
 * 但一張地理教學地圖沒有基隆河是說不過去的，所以另外收了 27 條**編者挑選**的
 ...
 * ## 這個檔案管一件事
 *
 * **145 條河川的身分**——中文名、本站 id、水利署河川代碼、管理等級（或「主要支流」
 ...
 *   （水利署河川代碼）選取，見下面的 `ref` 欄與 `RIVER_OSM_REFS`。全部 145 條。
...
export const RIVERS = {
  ...
  // ── 主要支流 27 條（不在公告的 118 個水系裡，見上面的說明）─────────
   ...
   * 英數字沒問題；`BASIN_IDS` 的 `ref.endsWith("000")` 也自然把它排除掉（支流
   * 沒有自己的流域面）。要是哪天有人把代碼當數字處理，這一條會第一個爆。
   *
