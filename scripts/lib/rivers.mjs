 ...
 *
 * 但一張地理教學地圖沒有基隆河是說不過去的，所以另外收了 29 條**編者挑選**的
 ...
 * ## 這個檔案管一件事
 *
 * **147 條河川的身分**——中文名、本站 id、水利署河川代碼、管理等級（或「主要支流」
 ...
 *   （水利署河川代碼）選取，見下面的 `ref` 欄與 `RIVER_OSM_REFS`。全部 147 條。
 * - `tw-basins`（面）＝ 水利地理資訊服務平台的「河川流域範圍圖」SHP，見 `BASIN_URL`。
 *   **只有獨立水系有流域面**（`category` 不是「主要支流」的那些），支流沒有，
 *   所以 `BASIN_IDS` 只從那 118 筆衍生；而那 118 筆裡上游也只發布了 72 筆，見 `BASIN_IDS` 的說明。
 */
...
export const RIVERS = {
  ...
  // ── 主要支流 29 條（不在公告的 118 個水系裡，見上面的說明）─────────
  // 淡水河水系：臺北盆地的三條河，課本講盆地與都市發展一定會點名
  基隆河: { id: "jilong-river", ref: "114030", category: "主要支流", parent: "淡水河" },
  /**
   * ⚠️ **`ref` 是 `11403H`，全站唯一一個不是 6 位純數字的河川代碼**（基隆河
   * `114030` 底下的支流編碼）。`fetchWaterwaysByRef()` 是用正規表示式精確比對，
   * 英數字沒問題；`BASIN_IDS` 是依 `category` 排除支流，也不受代碼格式影響。
   * 要是哪天有人把代碼當數字處理，這一條會第一個爆。
   *
   * ⚠️ **OSM 這個關聯只有 7.1 km，維基百科寫 19 km，兩個都對。** 外雙溪依定義
   * 是「與菁礐溪會合之後」才叫外雙溪，上游那段叫內雙溪、在 OSM 上是另一個關聯
   * （`11403S`），而維基百科的 19 km 是從擎天崗一路量下來的。**不要為了讓數字
   * 對起來把 11403S 也接進來**——那會讓圖上的「外雙溪」多出一段不叫外雙溪的河。
   */
  外雙溪: { id: "waishuang-river", ref: "11403H", category: "主要支流", parent: "淡水河" },
  ...
  羅東溪: { id: "luodong-river", ref: "256010", category: "主要支流", parent: "蘭陽溪" },
  /**
   * ⚠️ **冬山河的代碼是 `255000`，結尾是 000——但它在這裡是「主要支流」。**
   *
   * 255000 正好是宜蘭縣代碼序列裡的**空缺**（251 南澳、252 東澳、253 蘇澳、
   * 254 新城、[255 缺]、257 得子口、261 大溪川），因為它已經**改列區域排水**，
   * 不在民國 112 年 2 月公告的 118 個列管水系裡（公告明訂「凡屬排水管理辦法
   * 第 4 條規定之排水非屬河川」）。
   *
   * 收它是**編者的決定**，比照這一組其他 28 條：童玩節、親水公園與國際划船賽
   * 都在這條河上，課本與生活經驗都當它是一條河。水文上維基百科也說它與蘭陽溪
   * 在入海口附近匯流、同屬蘭陽溪水系，所以 `parent` 給蘭陽溪是誠實的。
   * 它「已經不是公告河川」這件事寫在內容檔與圖層說明裡，不藏起來。
   *
   * ⚠️ **也因為它，`BASIN_IDS` 與 tw-basins 的判準從「代碼結尾 000」改成
   * 「category 不是主要支流」**——原本那個是代理判準，剛好在所有支流身上成立，
   * 冬山河是第一個打破它的。不改的話它會被當成第 119 個「獨立水系」去找流域面。
   */
  冬山河: { id: "dongshan-river", ref: "255000", category: "主要支流", parent: "蘭陽溪" },
};
...
export const BASIN_IDS = Object.fromEntries(
  Object.entries(RIVERS)
     ...
     *
     * ⚠️ 判準是 **`category`，不是「代碼結尾 000」**。後者原本只是個剛好成立的
     * 代理判準，冬山河（`255000`，已改列區域排水、由編者收進「主要支流」）第一個
     * 打破它：照舊判準它會被當成第 119 個獨立水系去找流域面，而它根本沒有。
     * `build-geodata.mjs` 的 tw-basins transform 與日誌的「獨立水系」數也用同一條。
     */
    .filter(([, r]) => r.category !== "主要支流")
