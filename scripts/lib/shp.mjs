/**
 * 最小的 ESRI Shapefile（.shp）讀取器，支援 PolyLine 與 Polygon 兩種形狀。
 *
 * 為什麼需要它：水利地理資訊服務平台的河川圖資只提供 SHP，沒有 KML／GeoJSON
 * （水庫蓄水範圍那份反而只給 KML）。SHP 是固定寬度的二進位格式，不像 GML／KML
 * 那樣能用正規表示式對付，但格式本身很單純——比照 lib/gml.mjs／lib/kml.mjs 的
 * 既有決定，這裡也不加 shapefile 讀取器依賴（也沒有 ogr2ogr／GDAL 可用）。
 *
 * ⚠️ 這**不是**通用 SHP 讀取器，只認得 shape type **3（PolyLine，RIVERLIN）**與
 * **5（Polygon，BASIN）**——兩者的 record 二進位版面其實完全相同（Box + NumParts
 * + NumPoints + Parts[] + Points[]），差別只在语意上 parts 是線段還是環，所以共用
 * 同一段解析邏輯。遇到別的型別（點、Z/M 變體）會直接丟例外，不會靜默地把座標讀錯。
 *
...
const SHAPE_TYPE_LABELS = { [SHAPE_TYPE_POLYLINE]: "PolyLine", [SHAPE_TYPE_POLYGON]: "Polygon" };

/**
 * 剖析 .shp 檔案內容，回傳每筆 record 的多段線／環。
 *
 ...
 *
 * @param {Buffer} buf
 * @param {number} expectedShapeType SHAPE_TYPE_POLYLINE 或 SHAPE_TYPE_POLYGON
 * @returns {{ parts: [number, number][][] }[]} 每筆 record 的所有 part；
 *   Null shape（沒有幾何的記錄）回傳 `{ parts: [] }`。
 */
function parseShpParts(buf, expectedShapeType) {
  if (buf.readInt32BE(0) !== 9994) {
    throw new Error("不是有效的 .shp（file code 不是 9994）");
  }
  const fileShapeType = buf.readInt32LE(32);
  if (fileShapeType !== expectedShapeType) {
    throw new Error(
      `只支援 shape type ${expectedShapeType}（${SHAPE_TYPE_LABELS[expectedShapeType]}），拿到 ${fileShapeType}`,
    );
  }
  ...
  while (offset < buf.length) {
    ...
    if (shapeType === SHAPE_TYPE_NULL) {
      records.push({ parts: [] });
    } else if (shapeType === expectedShapeType) {
      ...
      throw new Error(
        `record 型別 ${shapeType} 不是 ${SHAPE_TYPE_LABELS[expectedShapeType]}（上游格式可能變了）`,
      );
    }
...
export const parseShpPolylines = (buf) => parseShpParts(buf, SHAPE_TYPE_POLYLINE);

/**
 * Polygon 版本。⚠️ 只處理**單一環、無孔洞**的多邊形（呼叫端應該自行檢查
 * `parts.length === 1`）——BASIN 圖資裡 26 個官方河川流域實測全部是這個形狀，
 * 沒有需要判斷外環／內環（洞）纏繞方向的案例，所以沒有實作那段邏輯，遇到
 * 多環的 record 應該讓呼叫端丟例外，不要猜哪個環是洞。
 */
export const parseShpPolygons = (buf) => parseShpParts(buf, SHAPE_TYPE_POLYGON);

/**
 * 最小的 .dbf 讀取器，只認 dBase III 的 Character（C）欄位。
 *
 * RIVERLIN 與 BASIN 的屬性表用到的欄位（`NAME`／`FROM`／`BASIN_NAME`…）都是文字，
 * 用不到 Numeric／Date 型別，所以沒有支援它們——遇到就會被當成文字讀出來，
 * 不會壞，只是沒意義（BASIN 的 `AREA` 目前也是當文字讀出來再自己 `Number()`）。
 * 真的需要別的型別時再補，不要預先猜。
 *
 * @param {Buffer} buf
