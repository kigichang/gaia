 ...
 * 但 BASIN.shp（河川流域範圍圖）的 .prj 寫的是 `TWD97_TM2_zone_121`——**投影坐標**，
 * 單位是公尺、以 121°E 為中央經線、false easting 250000——不能直接當經緯度使用，
 * 需要真的做反算橫麥卡托投影（inverse Transverse Mercator）。
 *
 ...
 * 反算公式轉回經緯度，四個測試點（含台北 101、玉山、以及 BASIN.shp 圖資本身的
 * bbox 四角）誤差都在 1e-8 度以下（浮點噪訊等級）。bbox 四角反算出來的範圍是
 * 北緯 21.9°–25.3°、東經 120.0°–122.0°，與臺灣本島＋周邊離島的實際地理範圍相符，
