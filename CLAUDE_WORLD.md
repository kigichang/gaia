...
- **編者選集**（20 筆，**九個圖層**）：原本是「作者精選」（`world-picks` 點）＋
...
1. **幾何仍然只有兩份**（`world-picks.geojson` 18 筆、`world-picks-areas.geojson` 2 筆）。
...
2. **內容 collection 九層共用 `world-picks`**（拆層當下那 19 份內容檔與它的分片一個字
   都沒動，id 在 collection 內唯一就夠；同月加的馬里亞納海溝直接放進同一個 collection，
   現在是 20 份）。實測九層每一層點第一筆，卡片、來源連結與面板標題
   都跟拆之前逐字相同。
