...
- **作者精選**（`world-picks` 點＋`world-picks-areas` 線）：編者挑的地方，目前是
  ...
  荷姆茲海峽、麻六甲海峽、土渕海峽、死海、裏海、貝加爾湖、東非大裂谷與百慕達三角。
...
那個緯度下 zoom 16 約 2 公尺一個像素，**實測那條水道在畫面上剛好 5 個像素寬**
（沿著西北－東南向斜穿過藍點，用 `queryRenderedFeatures` 逐像素打 `water` 圖層量出來的）。
再遠一級就完全看不出有水，而 16 也剛好讓 2.5 公里長的整條海峽橫躺在畫面上。
