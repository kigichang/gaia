...
⚠️ **`meta` 裡刻意塞了「台灣海峽」（台不是臺）。** 實測搜「台灣海峽」原本**一筆結果
都沒有**——`searchIndex.ts` 的 haystack 不做台／臺正規化，而那是臺灣使用者最常打的
寫法。⚠️ **這是全站性的缺口，不是這一筆特有的**：臺灣主題的「臺灣海峽中線」等圖層
同樣搜不到「台」字版。這次只用 `meta` 補了這一筆，**要根治得動 `searchIndex.ts` 的
haystack 正規化**（比照 `lib/crops.mjs` 既有的台／臺處理），那是另一件事。
...
39. **世界櫥窗：作者精選**（`/theme/world` → 世界櫥窗，見上）：
    ...
    - **臺灣海峽**：
      ...
      - 搜「臺灣海峽」「**台灣海峽**」「黑水溝」「福爾摩沙海峽」「Taiwan Strait」
        五種都要找得到——⚠️ 後四種只寫在 `meta` 上，其中「台灣海峽」是**台／臺**
        那個全站缺口的回歸判準（見上）
    - **土渕海峽**：
      ```js
