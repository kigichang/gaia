...
⚠️ **收進小視窗不等於可以省略。** 這些警語是內容誠信的承諾（比照 GBIF 觀測點與 ERA5 氣候值），所以**詳情卡不分**：`DetailCard` 的 `fullDescription()` 仍然把 `description` + 全部 `notes` 接起來給 `FeatureCard`／`QuakeCard`——那張卡是沒有內容檔的圖徵唯一看得到說明的地方。加新圖層時如果把警語漏回 `description`，畫面上只會顯示成「這個圖層的說明比較長」、按鈕不會出現，所以那兩條檢查不要拿掉。
