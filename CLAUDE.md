...
⚠️ **走 `FeatureCard` fallback 的圖層用 `detail.hideLayerDescription` 退掉這一段。** 判準跟 `QuakeCard` 一樣：**圖徵很多、而且那段字在每一張卡上逐字相同**。目前掛著的有四個：
...
| `tw-counties`、`tw-county-halls` | 22 筆都有內容檔，**今天是 no-op**；掛著是為了規則一致——新增縣市而內容檔還沒寫時，卡片會是「名稱＋來源」而不是整片圖層說明 |

⚠️ **有內容檔的圖徵完全不受影響**（`FeatureCard` 只在沒有內容檔時才走 fallback）：實測濁水溪、嘉義縣的卡片逐字未變。ODbL 要求的 `OpenStreetMap` 署名也還在——那是 `sources`，不是 description。

⚠️ **剩下三個同型的圖層目前沒掛**（`tw-basins` 72 筆／46 筆無內容檔、`tw-protected-areas` 53 筆／43 筆無內容檔、`tw-territory` 16 筆全部有內容檔）。要不要一起掛是內容判斷，不是技術問題：那幾層的圖層說明比較短，而且**是那張卡上唯一講得出東西的內容**。
