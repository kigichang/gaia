...
| `tw-protected-areas` | 53 處裡 43 處沒有內容檔（十座國家公園以外的保留區／保護區）；現在是「名稱／類別・約 N 公頃／資料來源」（2026-08 使用者反映跟抽屜重複後掛上） |
...
16. **國家公園與保護區**（`/theme/taiwan`，勾「國家公園與保護區」）：
    ...
    - 點大武山自然保留區 → **沒有內容檔**，走 `FeatureCard` fallback：顯示 geojson 的
      name、次標「自然保留區・約 N 公頃」與 sources，**不該再出現圖層說明**
      （`detail.hideLayerDescription`，見上）——那段字 43 處逐字相同、跟抽屜重複
