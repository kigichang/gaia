...
| 「火山大賞」的收錄名單 | `tvo.ncree.niar.org.tw/closely/landscape/global`（大屯火山觀測站 TVO，國家地震工程研究中心）＋維基百科〈十年火山〉（IAVCEI 的 16 座） | **程式完全不呼叫**。兩份都是名錄不是資料集；名單人工轉錄，18 筆的座標與海拔則逐位元取自 `volcanoes.geojson`（黃石除外，它不在 GVP 的全新世名單裡），寫進 `public/data/geo-manual/volcano-awards.geojson`，見 `CLAUDE_WORLD.md` |
...
- **⚠️ `sources` 的每一筆都要連得到「那份資料本身」，不是機構首頁**，而且**標籤要指名那份資料**。這條規則在 2026-08 掃過整個世界地理主題（51 個標籤）之後才寫死，因為抽查出三種都會讓人追不到出處的寫法：
  ...
  - **抄隔壁圖層填的來源**：`world-agriculture`（當時還是 planned）掛著 `Natural Earth`，但 NE 根本沒有農業資料。planned 的圖層也要填**做得出那一層的**真實資料集。⚠️ 這一筆後來還錯第二次：改填的「FAO 全球農業生態區（GAEZ）」同樣是憑印象寫的——GAEZ 只發 GeoTIFF，而且是生物物理的**潛力**分區不是實際的農業系統。2026-09 真的做出這一層時才換成 FAO SOLAW 那份。**填 planned 圖層的來源之前，先確認那份資料真的存在、真的拿得到、而且真的是那一層要畫的東西。**
...
npm run build:geodata -- --force --only=agriculture-paddy    # 世界主要農業帶六個大類要各跑一次
