...
> 踩過的坑與第 15–27、41–42、48–51 項驗證清單。本檔案只留跨主題的部分。
>
...
| 臺灣縣市界線 | `data.gov.tw/api/v2/rest/dataset/7442` → TGOS 的 GML zip（內政部國土測繪中心） | **只在建置期呼叫**，政府資料開放授權條款第 1 版。⚠️ **「海岸地形」也吃這一份**——本島外環是把 19 個縣市的環有向邊相消併出來的，見 `CLAUDE_TW.md` |
...
| 「火山大賞」的收錄名單 | `tvo.ncree.niar.org.tw/closely/landscape/global`（大屯火山觀測站 TVO，國家地震工程研究中心） | **程式完全不呼叫**。那一頁是一份教材，不是資料集；名單人工轉錄，六筆的座標與海拔則逐位元取自 `volcanoes.geojson`（黃石除外，它不在 GVP 的全新世名單裡），寫進 `public/data/geo-manual/volcano-awards.geojson`，見 `CLAUDE_WORLD.md` |
...
| `quakes-strongest-points` | circle（規模最強的 18 次地震；同一個 `hazard` 中性色但畫得更深、更大、有白框，比照 `tw-quakes-major` 疊在 `tw-quakes` 上的那一組參數。⚠️ 全站第二個「非世界櫥窗」的 `exemptFromMaxActive`，見 `CLAUDE_WORLD.md`） |
| `volcanoes-points` | circle（1,214 座活火山，`strokeWidth: 0`；半徑固定不隨 zoom 變） |
| `volcano-awards-points` | circle（**火山大賞**：六座「憑一件事上榜」的火山。同一個 `volcano` 洋紅但半徑 7、有白框，比照 `quakes-strongest` 疊在 `quakes` 上的那一組參數。⚠️ 全站第三個「非世界櫥窗」的 `exemptFromMaxActive`，見 `CLAUDE_WORLD.md`） |
...
| `world-cyclone-centers-points` / `-label` | circle + symbol（附屬圖層；1,729 個中心定位點，**跟母圖層共用 id**，半徑吃 zoom 與風速、顏色是薩菲爾—辛普森的色階；標註只在該氣旋被選取時出現） |
...
| `tw-coasts-line` / `tw-coasts-label` | line + symbol（本島海岸線切成四段；**沿用水系藍**，⚠️ `relief` 洋紅是先試過才否掉的（東部海岸與海岸山脈稜線中位數只差 6.3 km），見 `CLAUDE_TW.md`） |
...
⚠️ **子項目本身也可能是一個要解釋的概念，那時要宣告 `items.detail`。** 點抽屜裡的子項目名稱（以及搜到子項目本身時）預設開的是**母圖層的** `detail`，而那不一定講得通：古蹟母圖層的 `detail` 是 `monument`（點圓點要開的是那一處古蹟的卡），但點「國定古蹟」四個字要開的是**那個級別的定義**。`items.detail`（目前只支援 `type: "geo"`）就是覆蓋這條路徑用的，內容檔放 `src/content/geo/<collection>/<item id>.json`，`validate-content.mjs` 兩個方向都會擋。判準是**「子項目名稱回答的問題」跟「圖徵回答的問題」不同**：古蹟（級別的定義 vs 那一處古蹟）、主要作物分布（那一類作物 vs 那個鄉鎮）與**柯本氣候分區**（A 熱帶氣候這個大類是什麼 vs 我腳下這一格是 Af 還是 Aw）都成立，特有種與垂直植被帶不成立、不必宣告。⚠️ **不宣告的後果是一張完全靜默的空白面板**（item id 拿去母圖層那一支查 geojson 一定查不到 → 卡片回 `null`），詳見 `CLAUDE_TW.md` 的古蹟那一節。⚠️ 柯本那一層更空：它同時有 `hideLayerDescription`，所以在補上 `items.detail` 之前，點「A 熱帶氣候」開出來的整張卡只有圖層名稱五個字加一行資料來源（實測）。
...
├─ .map-top-left      MapSearchBox 搜尋藥丸（[☰][輸入框][✕][🔍]）+ 建議清單，右邊接 FeedbackButton
...
`.map-top-left` 是 `display: flex` 的一列，`MapSearchBox` 用 `flex: 1; min-width: 0` 佔滿剩下的寬度，`FeedbackButton`（`src/components/FeedbackButton.tsx`）用 `flex: none` 排在右邊——這是唯一一個用 `var(--accent)`（不是 `.map-fab` 預設的中性灰）的浮動按鈕，因為它要引導點擊而不是單純導覽。⚠️ 它一度是**寫死的**暖紅 `#e5484d`（那時是贊助用的心型）：紅色在這個站上讀起來像警示，對「回報問題」是誤導，而且寫死的色碼只有一個值、不隨明暗模式變。要換色請用既有的 token。抽屜開著時整欄一起 `visibility: hidden`，它也會跟著收起。

⚠️ **它不是連結，是第三個 `MapPopover`。** 這顆按鈕早期是一顆心型的 `<a target="_blank">`，直接跳到均一的贊助頁；2026-08 先改成「問題、建議與贊助」的小卡，之後贊助那一段整個移除，現在只剩「問題回報」（GitHub Issue 與 `mailto:` 兩個連結）。做成小卡而不是外跳的理由是**回報問題在整個站上沒有別的入口**——主題頁是滿版地圖，沒有頁首也沒有頁尾，直接外跳等於把那條路徹底藏起來，而且回報有兩條路，一顆按鈕沒辦法同時直連兩個地方。

⚠️ **`flex: none` 要掛在 `MapPopover` 的根節點上（`.map-feedback-root`），不是按鈕本身上。** 這就是 `MapPopover` 那個 `rootClassName` 存在的唯一理由：flex 子元素是根節點、按鈕只是它的孫節點，掛錯地方時搜尋框一長按鈕就被壓扁。窄螢幕那條 `display: none` 同理（見下）。
...
⚠️ **窄螢幕（≤860px）由 ⋮⋮⋮ 選單接手同一段內容，兩者永遠只出現一個。** 按鈕在那個寬度整顆是 `display: none`（要讓出 ⋮⋮⋮ 的位置與打字空間，見下），而**回報問題在這個站上沒有別的入口**——少了這一段，手機上就完全沒有地方回報。所以內文抽成 `ContactNote`（`src/components/ContactNote.tsx`）給兩邊共用，`AppMenu` 那一段包在 `.map-menu-contact` 裡預設 `display: none`、只在窄螢幕變 `block`。

⚠️ **`.map-feedback-root` 與 `.map-menu-contact` 那兩條 CSS 是一組的，改一條就要改另一條**——只改其中一條的後果分別是「手機上沒有回報入口」或「桌機上同一段內容講兩次」，兩者在建置期都抓不到。窄螢幕還要把 `.map-menu` 加寬到 300px：⋮⋮⋮ 面板本來只靠 `min-width: 220px` 撐著，完整網址在那個寬度下會斷得很碎。

⚠️ `/compare` 的 `variant="inline"` **沒有**這一段（那裡是頁首的一列，塞不下兩段文字），這是已知的。

⚠️ **窄螢幕（≤860px）的 `.map-top-left` 會變成滿寬，於是它跟右上角的 `.map-top-right`（⋮⋮⋮，絕對定位的另一欄）重疊，而它的 `z-index` 比較高——所以蓋掉的一定是主題選單那顆按鈕，而且是連看都看不到、也點不到**（實測 500px 寬時整顆被搜尋藥丸的右端吃掉；使用者在 Pixel 7a 上的回報是「愛心蓋住選主題的按鈕」）。修法有兩條、缺一不可：媒體查詢裡替這一欄補 `padding-right: calc(var(--top-right-fab-w) + var(--fab-gap) + 8px)`（`--top-right-fab-w` 是那顆 fab 的寬度 46px＝圖示 20 + padding 12×2 + 框線 1×2），並且把 `.map-feedback-root`（`MapPopover` 的根節點，不是按鈕本身）整個 `display: none`——讓出那段留白之後，412px 的手機上搜尋框只剩三百出頭，再擺一顆 46px 的按鈕就不夠打字了。⚠️ **只藏那顆按鈕是不夠的**，搜尋藥丸自己一樣會蓋住 ⋮⋮⋮。驗法是 `document.elementFromPoint()` 打在 ⋮⋮⋮ 的正中心要命中那顆按鈕，光看截圖上那顆按鈕不見了會誤判成修好了。
...
索引是 **lazy 的**：搜尋框第一次獲得焦點才 `buildSearchIndex()`。**實測（2026-08，production build 讀 `performance.getEntriesByType('resource')`）它會多抓 28 份、合計約 3.05 MB**（⚠️ 那次實測含「世界之最」兩層的 33 KB，該組下架後是 26 份、約 3.02 MB；**再加上 2026-08 新增的「重要濕地」197 KB 與「世界紀錄熱帶氣旋路徑」33 KB 是 28 份、約 3.25 MB；**2026-09 再加上「規模最強地震」7 KB 與「火山大賞」4 KB 是 30 份、約 3.27 MB**）。⚠️ 熱帶氣旋那一層的 **268 KB 中心定位點不在裡面**，那是附屬圖層而它沒有宣告 `browse`（比照颱風的中心定位點，見下）：
...
npm run build:geodata -- --force --only=tw-coasts              # 本島海岸線四段（重抓縣市界 GML 再相消）
...
npm run build:geodata -- --force --only=quakes-strongest       # 規模最強的 18 次地震（世界主題）
                                       # ⚠️ 名單是人工抄的、規模與座標查 USGS；14 筆對得到、
                                       #    4 筆（1575／1668／1755／1833）ComCat 沒有，見 lib/quakes-strongest.mjs
...
npm run build:geodata -- --force --only=world-cyclones       # 33 個世界紀錄熱帶氣旋的路徑
                                       # ⚠️ 上游那份 CSV 有 331 MB，會跑好幾分鐘（逐塊串流過濾，不是卡住）
npm run build:geodata -- --force --only=world-cyclone-centers # 同一份下載的 1,729 個中心定位點
                                       # ⚠️ 兩個分開跑就會下載兩次；不帶 --only 讓它跑完整個 SOURCES
                                       #    才會共用同一次下載（見 lib/cyclones.mjs 的 module-level 快取）
...
> **第 15–27 項、第 41–42 項與第 48–51 項是臺灣主題各圖層的驗證，搬到 `CLAUDE_TW.md` 了**
...
└─ components/
   ...
   ├─ MapPopover.tsx      # ⋮⋮⋮、「圖層」磚與問題回報小卡共用的泡泡容器（rootClassName 見上）
   ├─ MapLayersPopover.tsx# 左下「圖層」磚（內容重用 LayerToggles）
   ├─ MapSearchBox.tsx    # 左上搜尋藥丸（含開抽屜的 ☰）與建議清單
   ├─ FeedbackButton.tsx  # 搜尋藥丸右邊的「問題回報」：開一張小卡（寬螢幕才有）
   ├─ ContactNote.tsx     # 那張卡的內文（Issue／mailto）。⚠️ 窄螢幕改由 ⋮⋮⋮ 選單顯示，共用這一份
...
├─ geo-content/*.json     # 地理要素說明的分片，42 份（build:geo-content 產生，禁止手改）
...
├─ lib/coasts.mjs          # 本島海岸線四段的存取層（縣市界 GML → 有向邊相消 → 依四個分界點切段）
...
├─ lib/cyclones.mjs       # 世界紀錄熱帶氣旋的存取層（33 筆的 IBTrACS 序號對照表、331 MB CSV 的逐行串流過濾、
│                         #   ±180 切段、與紀錄表的氣壓／風速交叉檢查）
