...
⚠️ **144 條現在全部都有內容檔了**（見下一節），所以再也沒有「走 fallback 撿圖層 sources」這條退路：**新增或重寫任何一份河川內容檔時漏掉 `OpenStreetMap`，那張卡片的署名就會憑空消失，而且畫面上不會有任何錯誤**。
...
| `tw-rivers`、`tw-protected-areas`、`tw-counties`、`tw-county-halls` | 144／53／22／22 筆**都有內容檔**，所以**今天都是 no-op**；掛著是為了規則一致——新公告一條河或一處保留區而內容檔還沒寫時，卡片會是「名稱＋類別＋來源」而不是整片圖層說明。（保護區那 43 份是 2026-08 補的，見下一節；河川最後那 97 份是用維基百科補的，見「另外 97 條沒有官方詳細資料」） |
...
18. **臺灣河川**（`/theme/taiwan`，勾「臺灣河川」）：
    ...
    - 點濁水溪（有內容檔）→ 卡片有幹流長度／流域面積／分類、發源地、出海口，
      ...
      顯示的是內容檔的 `sources`，所以 144 份內容檔都必須帶這一筆），以及帶河名的
    ...
    - 144 條河川的 `id`／代碼／等級全部在 `scripts/lib/rivers.mjs` 的 `RIVERS`，
      而 **144 條全部都有內容檔**（`src/content/geo/tw-rivers/`）——`npm run validate` 會交叉檢查
