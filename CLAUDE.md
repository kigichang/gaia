...
河川線（OSM，ODbL）畫在臺灣通用電子地圖（NLSC，政府資料開放授權條款第 1 版）的圖磚上，兩份資料**各自獨立並存、未經融合**，屬 ODbL 的「集合資料庫（Collective Database）」，只需各自標示，不會觸發「衍生資料庫須以 ODbL 免費提供」的義務。因此 `tw-rivers` 的 47 份內容檔都把 `OpenStreetMap` 加進 `sources`——`FeatureCard` 有內容檔時顯示的是內容檔的 `sources`（`feature?.sources ?? fallback.sources`），只寫在圖層定義上是**看不到的**。剩下 97 條沒有內容檔，走 fallback 顯示圖層自己的 `sources`，那一份本來就有 `OpenStreetMap`；**之後替它們補內容檔時要記得一起帶上這一筆**，否則那張卡片的署名會憑空消失（補那 21 條縣市管河川時就踩在這條規則上）。
...
| `tw-rivers` | 144 條裡 97 條沒有內容檔；現在是「乾華溪／阿里磅溪・新北市／直轄市管河川／資料來源」 |
...
18. **臺灣河川**（`/theme/taiwan`，勾「臺灣河川」）：
    ...
    - 144 條河川的 `id`／代碼／等級全部在 `scripts/lib/rivers.mjs` 的 `RIVERS`，
      其中 47 條要對得上內容檔（`src/content/geo/tw-rivers/`）——`npm run validate` 會交叉檢查
