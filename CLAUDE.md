...
百慕達三角是一片海域，畫成點就等於把它唯一的地理內容丟掉。

⚠️ **範圍那一層是 `reference` 中性灰虛線，不是分類色**，而且那條虛線是必要的：
百慕達三角**沒有官方界線**（美國海軍與海岸防衛隊明講它不在任何官方海圖上），實線會
讓它看起來像一條真實的界線，而那正好是這張卡要拆穿的東西。因此它跟同主題的緯度
參考線、國際換日線長得一樣——那是刻意的，它們確實是同一類東西，區辨靠沿線標註
（所以 `label` 在這一層也是必要條件）。
...
| `world-picks-areas-line` / `-label` | line + symbol（世界櫥窗；編者選集裡**有範圍**的項目，`reference` 中性灰虛線——那條虛線的語意是「別人是這樣圈的」，見下） |
